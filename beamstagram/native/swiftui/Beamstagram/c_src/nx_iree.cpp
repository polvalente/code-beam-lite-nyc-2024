...
iree_hal_device_t *nx_iree_create_device(char *device_uri) {
  ...
  iree_hal_device_t* device = create_device(driver_registry, std::string("local-sync://"));

  return device;
}

iree_vm_instance_t *nx_iree_create_instance() { return create_instance(); }

unsigned char *
nx_iree_image_call(iree_vm_instance_t *vm_instance, iree_hal_device_t *device,
                   uint64_t bytecode_size, unsigned char *bytecode,
                   uint64_t *input_dims, unsigned char *input_data,
                   char *error_message, uint32_t seed, float noise_amount) {
  if (!input_data) {
    return nullptr;
  }
  ...
  inputs.push_back(new iree::runtime::IREETensor(
      input_data, input_size, input_dims_vec,
      iree_hal_element_types_t::IREE_HAL_ELEMENT_TYPE_UINT_8));

  input_dims_vec.clear();

//  inputs.push_back(new iree::runtime::IREETensor(
//      &seed, 4, {}, iree_hal_element_types_t::IREE_HAL_ELEMENT_TYPE_UINT_32));
//
//  inputs.push_back(new iree::runtime::IREETensor(
//      &noise_amount, 8, {},
//      iree_hal_element_types_t::IREE_HAL_ELEMENT_TYPE_FLOAT_32));

  // driver name is hardcoded because there is only a check for CUDA
  auto [status, optional_result] =
      call(vm_instance, device, "not_cuda", bytecode,
           static_cast<size_t>(bytecode_size), inputs);
...
char **nx_iree_call(iree_vm_instance_t *vm_instance, iree_hal_device_t *device,
                    uint64_t bytecode_size, unsigned char *bytecode,
                    uint64_t num_inputs, char **serialized_inputs,
                    uint64_t num_outputs, char *error_message,
                    uint64_t *output_byte_sizes) {
  std::vector<iree::runtime::IREETensor *> inputs;
